...
from .models.backbones.clip.models import CLIPTextContextEncoder
...
class Difseg(nn.Module):
    def __init__(self, context_num,
        ...
        self.text_encoder = CLIPTextContextEncoder(embed_dim=512)
        ...
        context_num = context_num  # context prompt number
        self.texts = tokenize("water", context_length=self.context_length).to('cuda')
        self.contexts = nn.Parameter(torch.randn(context_num, prompt_num, 512))
        self.query_embed = nn.Embedding(context_num, 256)
        text_in_dim = 512
        text_out_dim = 256
        self.text_proj = nn.Sequential(
            nn.Linear(text_in_dim, text_out_dim), nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        text_embeddings = self.text_encoder(self.texts, context=self.contexts).unsqueeze(0).expand(b_size, -1, -1)
        query_feat = self.text_proj(text_embeddings) #.permute(1, 0, 2)
        ...
        query_embed = self.query_embed.weight.unsqueeze(0).repeat(
            (b_size, 1, 1))
        sparse_embeddings = query_feat + query_embed
